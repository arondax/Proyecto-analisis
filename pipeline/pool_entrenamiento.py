...
Tres funciones públicas, cada una con su propio disparador:
    ...
    - procesar_muestreo(candidatos_ultima_partida)
      Se llama UNA vez al final de cada ejecución de ingesta
      (script_datos.py), con la lista acumulada de compañeros vistos en
      la última partida de cada jugador activo procesado en ese run.
...
def procesar_muestreo(candidatos_ultima_partida):
    """
    candidatos_ultima_partida: lista de tuplas (nombre, tag, region) con
    los compañeros vistos en la ÚLTIMA partida de cada jugador activo
    procesado en esta ejecución. Puede haber repetidos entre distintos
    jugadores trackeados (se deduplican aquí).
 
    Excluye a quien ya tenga activo=true, elige un porcentaje aleatorio
    entre PORCENTAJE_MUESTREO_MIN y MAX sobre el resto, y añade/reactiva
    respetando MAX_JUGADORES_ENTRENAMIENTO.
    """
    ...
    candidatos_unicos = {}
    for nombre, tag, region in candidatos_ultima_partida:
