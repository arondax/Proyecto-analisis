...
- amigo_recurrente: candidato que ha aparecido >= UMBRAL_CANDIDATO_A_AMIGO
...
def registrar_compañeros(nombres_tags, region):
    ...
    for nombre, tag in nombres_tags:
        ...
        if clave in claves_entrenamiento:
            continue  # ya pasó de tier, no hace falta seguir contando
...
def evaluar_promocion_amigos():
    ...
    alcanzado UMBRAL_CANDIDATO_A_AMIGO apariciones. Pensado para
    ...
    for c in candidatos.get('candidatos', []):
        if c['apariciones'] >= UMBRAL_CANDIDATO_A_AMIGO:
